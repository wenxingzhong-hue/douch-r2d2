...
// 安全提取并解析各类日期/时间字段（未填时间或不规范的，赋予 Infinity 使其安全排在最后面）
function getTaskTime(task) {
    if (!task) return Infinity;
    const val = task.time || task.date || task.datetime || task.endTime || task.dateTime || task.completionTime;
    if (!val) return Infinity;
    const ts = Date.parse(val);
    return isNaN(ts) ? Infinity : ts;
}

// 严谨的时间升序比较器（时间越早越靠上，Infinity 自动沉底）
function compareTimes(aTime, bTime) {
    if (aTime === bTime) return 0;
    if (aTime === Infinity) return 1;  // a没时间，排到后面去
    if (bTime === Infinity) return -1; // b没时间，排到后面去
    return aTime - bTime; // 升序排列：时间小的（早的）在前
}
...
app.get('/api/task/:uid', async (req, res) => {
    try {
        ...
        // 【步骤一】一级任务严格升序排序：完成时间早的（如6月1日）排在时间晚的（如7月1日）上面
        ...
        if (userTasks.l2 && Array.isArray(userTasks.l2)) {
            // 构建一个高度容错的一级任务时间映射 Map（同时支持用 ID 或 名字 匹配绑定）
            ...
            function getParentL1Time(task) {
                ...
                return Infinity; // 如果找不到对应的一级任务，默认让它排在最后面
            }
